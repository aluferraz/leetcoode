...
var earliestFullBloom = function (plantTime, growTime) {
    ...
    let nextPlantQueue = [];//new PriorityQueue(nextFlowerfunction);
    for (let i = 0; i < plantTime.length; i++) {
        nextPlantQueue.push({ idx: i, plant: plantTime[i], grow: growTime[i] });
    }
    nextPlantQueue.sort((a, b) => b.grow - a.grow);

    // while (nextPlantQueue.size() > 0) {
    for (let i = 0; i < nextPlantQueue.length; i++) {
        let element = nextPlantQueue[i];
        plantDays += element.plant;
        earliest = Math.max(earliest, (plantDays + element.grow));
...
function nextFlowerfunction(a, b) {
    //{ priority: (flower) => (flower.plant - flower.grow) }
    let aPrio = (a.plant - a.grow);
    let bPrio = (b.plant - b.grow);
    if (aPrio === bPrio) {

        let effectiveACostStartingByB = ((a.plant - b.grow) + b.plant);
        let effectiveBCostStartingByA = ((b.plant - a.grow) + a.plant);
        if (effectiveACostStartingByB < effectiveBCostStartingByA) return 1;
        return -1;
    }
